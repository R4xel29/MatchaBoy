...
export async function geocodeAddressDirect(query: string, baseLat: number, baseLng: number): Promise<any[]> {
  try {
    ...
    if (query) {
      ...
      const url = `${NOMINATIM_BASE}/search?q=${encodeURIComponent(query)}&format=json&addressdetails=1&extratags=1&namedetails=1&limit=8&accept-language=id&countrycodes=id&viewbox=${left},${top},${right},${bottom}&bounded=1`;
