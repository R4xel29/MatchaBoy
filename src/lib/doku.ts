...
export function generateQrisString(amount: number, orderId: string, customNmid?: string): string {
  ...
  if (cleanNmid.startsWith('26') || cleanNmid.startsWith('51')) {
    qris += cleanNmid;
  } else {
    ...
    const subTag00 = "00" + String(guid.length).padStart(2, '0') + guid;
    const subTag01 = "01" + String(nmidVal.length).padStart(2, '0') + nmidVal;
    const subTag02 = "02" + String(criteria.length).padStart(2, '0') + criteria;
    
    const subTags = subTag00 + subTag01 + subTag02;
    qris += '51' + String(subTags.length).padStart(2, '0') + subTags;
  }
