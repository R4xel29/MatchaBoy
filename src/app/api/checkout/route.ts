    ...
    try {
        ...
        if (isDoku && paymentSettings) {
            try {
                ...
                // Note: For DOKU+QRIS, the real QRIS is generated by DOKU on their hosted checkout page.
                // We do NOT generate a local QRIS here because it's not registered in the GPN network.
                await prisma.order.update({
                    where: { id: order.id },
                    data: { 
