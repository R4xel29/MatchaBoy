        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
               ...
               className="relative w-full max-w-md max-h-[90vh] bg-white rounded-2xl shadow-xl overflow-hidden z-10 flex flex-col"
             >
               {/* Modal Header */}
               <div className="flex justify-between items-center px-5 py-4 border-b border-border/30 bg-white shrink-0">
                  <h2 className="text-lg sm:text-xl font-heading font-bold text-foreground overflow-hidden truncate">{editingPopup ? 'Edit Promo Popup' : 'Tambah Promo Popup'}</h2>
                  <button disabled={loading} onClick={() => setIsModalOpen(false)} className="p-1.5 text-muted-foreground hover:bg-muted rounded-xl transition-colors"><X className="w-5 h-5"/></button>
               </div>
               ...
               {/* Modal Footer */}
               <div className="flex justify-between items-center p-4 sm:px-6 border-t border-border/30 bg-gray-50/50 shrink-0">
                  <button 
                    type="button"
