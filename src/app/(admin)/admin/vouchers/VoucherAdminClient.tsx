...
export default function VoucherAdminClient({ 
  ...
  const handleUpdateSystemVoucherCode = async (key: string, code: string) => {
    try {
      const res = await fetch('/api/admin/vouchers/system', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6 font-sans text-gray-800 pb-20">
      ...
      <div className="flex border-b border-gray-200 mb-8 gap-6 shrink-0">
        <button
          onClick={() => setActiveTab('templates')}
          className={`pb-4 px-1 text-sm font-bold transition-all relative cursor-pointer ${
            activeTab === 'templates' ? 'text-[#B48A5E]' : 'text-gray-400 hover:text-gray-650'
          }`}
        >
          Semua Template Voucher
          {activeTab === 'templates' && (
            <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#B48A5E]" />
          )}
        </button>
        <button
          onClick={() => setActiveTab('system')}
          className={`pb-4 px-1 text-sm font-bold transition-all relative cursor-pointer ${
            activeTab === 'system' ? 'text-[#B48A5E]' : 'text-gray-400 hover:text-gray-650'
          }`}
        >
          Voucher Sistem (Reward)
          {activeTab === 'system' && (
            <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#B48A5E]" />
          )}
        </button>
      ...
      {activeTab === 'templates' ? (
        ...
        /* Voucher Sistem (Reward) Tab content */
        <div className="space-y-6">
          <div className="bg-amber-500/[0.03] border border-amber-500/10 p-5 rounded-3xl text-xs text-amber-900 leading-relaxed flex items-start gap-3">
          ...
          {loadingSystem ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3 bg-white rounded-3xl border border-gray-100 shadow-sm">
              ...
              <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Memuat voucher sistem...</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {systemVouchers.map((v) => {
                ...
                return (
                  <div key={v.key} className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden flex flex-col justify-between p-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-serif font-black text-gray-900 leading-tight">{v.label}</h3>
                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">Key: {v.key}</p>
                          </div>
                        </div>
                        ...
                        {hasTemplate ? (
                          ...
                          <span className="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-100 rounded-full text-[9px] font-bold uppercase tracking-wider">
                            Belum Dibuat
                          </span>
                        )}
                      </div>
                      ...
                      {hasTemplate ? (
                        <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 space-y-2 text-xs">
                          <div className="flex justify-between">
                            <span className="text-gray-400 font-semibold">Kode Template:</span>
                            <span className="font-mono font-bold text-amber-800 bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded text-[10px]">{v.template.code}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400 font-semibold">Tipe:</span>
                            <span className="font-bold text-gray-700">{v.template.type === 'DISCOUNT_RP' ? 'Diskon Nominal' : v.template.type === 'DISCOUNT_PCT' ? 'Diskon Persen' : v.template.type}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400 font-semibold">Nilai Diskon:</span>
                            <span className="font-bold text-gray-900">
                              {v.template.type === 'DISCOUNT_PCT' ? `${v.template.discountValue}%` : formatRupiah(v.template.discountValue)}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400 font-semibold">Min. Belanja:</span>
                            <span className="font-bold text-gray-700">{formatRupiah(v.template.minPurchase)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400 font-semibold">Total Klaim:</span>
                            <span className="font-bold text-gray-700">{v.template._count?.vouchers ?? 0}</span>
                          </div>
                        </div>
                      ) : (
                        <div className="bg-amber-50/30 rounded-2xl p-4 border border-amber-100/50 text-xs text-amber-800 leading-relaxed">
                          ⚠️ Template default untuk key ini belum dibuat di database. Tombol di bawah akan menginisiasi template default secara otomatis.
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-6 space-y-3">
                      {hasTemplate ? (
                        <>
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="flex-1 py-2.5 rounded-xl bg-[#B48A5E] hover:bg-[#946F48] text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-sm"
                            >
                              <Edit className="w-3.5 h-3.5" />
                          ...
                          <div className="pt-3 border-t border-gray-100">
                            <label className="text-[9px] font-bold text-gray-400 uppercase block mb-1.5">Ganti Template Mapped</label>
                            <select
                              value={v.activeCode}
                              onChange={(e) => handleUpdateSystemVoucherCode(v.key, e.target.value)}
                              className="w-full text-xs font-semibold bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-gray-700 focus:outline-none focus:border-[#B48A5E]"
                            >
                              {templates.map(t => (
                        ...
                        <button
                          ...
                          className="w-full py-2.5 rounded-xl bg-[#B48A5E] hover:bg-[#946F48] text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-sm"
                        >
                          <Plus className="w-4 h-4" />
                          <span>Inisiasi Template Default</span>
                        </button>
                      )}
                    </div>
                  </div>
