...
export default function CustomerDisplayClient() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none overflow-hidden">
      {/* Header Bar */}
      <header className="h-16 px-8 bg-slate-900/80 border-b border-slate-800/80 backdrop-blur-md flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium">Customer Facing Display</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {displayState?.customerName && (
            <div className="px-3 py-1.5 rounded-full bg-slate-800/80 border border-slate-700/60 text-xs font-semibold text-amber-300 flex items-center gap-2">
          ...
          <AnimatePresence mode="wait">
            {isCompleted ? (
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
                className="my-auto bg-slate-900/90 border border-amber-500/30 rounded-3xl p-8 shadow-2xl backdrop-blur-xl max-w-md mx-auto w-full text-center space-y-6 relative overflow-hidden"
              >
                ...
                <div>
                  ...
                  <h3 className="text-2xl font-black text-white">{formatRupiah(totalPayable)}</h3>
                </div>

                {/* QR Code Frame */}
                <div className="p-4 bg-white rounded-2xl shadow-xl inline-block border-4 border-amber-400/80 mx-auto relative group">
                  {settings.qrisImage ? (
                    <img
