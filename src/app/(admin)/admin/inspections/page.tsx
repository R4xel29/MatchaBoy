...
export default async function AdminInspectionsPage() {
  ...
  // Jika bukan ADMIN (Karyawan / CASHIER), tidak perlu membebankan query finansial & log audit lengkap
  let shifts: any[] = [];
  let ingredients: any[] = [];
  let recentMovements: any[] = [];
  let recentLogs: any[] = [];
  let todayChecklistLogs: any[] = [];
  ...
  if (isAdmin) {
    const [
      ...
      prisma.activityLog.findMany({
        where: {
          entity: { in: ['CHECKLIST_OPENING', 'CHECKLIST_CLOSING', 'CHECKLIST_ROUTINE', 'SOP_SUBMISSION'] },
    ...
    const enrichedShifts = await Promise.all(
      fetchedShifts.slice(0, 15).map(async (shift) => {
    ...
    todayChecklistLogs = fetchedTodayLogs;
  }
